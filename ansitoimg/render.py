...
def _doRichRender(ansiText: str, wide: bool = True, width: int = 49) -> Console:
    dynamic_width = round(width * 1.82) if wide else width

    console = Console(width=dynamic_width, record=True, file=StringIO())
...
def ansiToSVG(ansiText: str, fileName: str, theme: str | None = None, wide: bool = True, width: int = 49):
    ...
    console.save_svg(
        fileName, theme=_doRichTerminalTheme(theme), code_format=CODE_FORMAT, title=""
    )
